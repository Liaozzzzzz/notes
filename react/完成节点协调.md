# completeUnitOfWork的工作

在遍历fiber节点时, 当某个节点不存在child节点时(next === null),需要调用改方法结束当前fiber节点的遍历, 并指向其父节点的兄弟节点进行下一轮遍历

```javascript
function completeUnitOfWork(unitOfWork: Fiber): Fiber | null {
  workInProgress = unitOfWork;
  do {
    // 对于第一次挂载的节点, current应该为null;
    const current = workInProgress.alternate;
    // 指向父节点, 遍历到RootFiber时为null; 
    const returnFiber = workInProgress.return;

    // 判断fiber计算是否完成, 如果因为一些错误导致没法完成计算, 需要卸载掉当前结果
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      // 调用completeWork函数, 传入current, workInProgress和renderExpirationTime参数,
      // 此处renderExpirationTime为外出函数设置的当前更新时间
      let next;
      if (
        !enableProfilerTimer ||
      ...
      // 重置fiber节点上的childExpirationTime为NoWork
      ...
      if (
        ...
        // side-effect order.
        // 将当前子节点的firstEffect挂载到父节点的firstEffect上
